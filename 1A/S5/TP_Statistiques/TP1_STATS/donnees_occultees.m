...
L = taille_ecran(3);
H = taille_ecran(4);
theta_1 = 2*pi*rand; %tirage alétoire de l'angle theta_1
theta_2 = 2*pi*rand; %tirage aléatoire de l'angle theta_2
...
if theta_1 <= theta_2 %la première condition
    indices = find((theta_donnees_bruitees>=theta_1 & theta_donnees_bruitees<=theta_2));
else
    indices = find((theta_donnees_bruitees>= 0 & theta_donnees_bruitees<= theta_2) | (theta_donnees_bruitees>=theta_1 & theta_donnees_bruitees<2*pi));
end
%récupérer les éléments de x_donnees_bruitees et y_donnees_bruitees qui
%vérifient les conditions posés par l'exercice.
x_new = x_donnees_bruitees(indices); 
y_new = y_donnees_bruitees(indices);

% Affichage du cercle reel :
plot(x_cercle_reel([1:end 1]),y_cercle_reel([1:end 1]),'r','LineWidth',3);

% Affichage des donnees bruitees :
plot(x_new,y_new,'k+','MarkerSize',10,'LineWidth',2);
axis(bornes);
