...
figure(11),clf
plot(0,0,'ko','linewidth',2)
hold on,plot([0 1.5*max(x)],[0 0],'k-') % tracer le segment de droite permettant de 
...
title('Des nombres suivant une distribution gaussienne sur le plan');
...
x = 3*randn(3,nb_indiv) + 2; 
x_mean = [mean(x(1,:)); mean(x(2,:)); mean(x(3,:))];
...
legend('individus','element moyen');
title("Des nombres suivant une distribution gaussienne dans l'espace");
